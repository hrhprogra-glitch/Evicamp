...
export const TablaProductos: React.FC<Props> = ({ 
  ...
  return (
    <div className="border border-[#E2E8F0] flex-1 flex flex-col bg-white relative">
      ...
      <div className="grid grid-cols-12 bg-[#1E293B] text-white p-4 text-[11px] font-black uppercase tracking-[0.2em] shrink-0 items-center">
              ...
              return (
                <div key={item.id} className="grid grid-cols-12 items-center p-4 border-b-2 border-[#1E293B] bg-[#F8FAFC] shadow-inner">
                  ...
                  <div className="col-span-1 flex justify-center pr-2">
                    {item.imageUrl ? (
                      <img src={item.imageUrl} alt="img" className="w-10 h-10 object-cover rounded border border-[#E2E8F0]"/>
                    ) : (
                      <div className="w-10 h-10 bg-[#E2E8F0] rounded flex items-center justify-center text-[#94A3B8]" title="Sin imagen">
                        <ImageIcon size={16}/>
                      </div>
                    )}
                  </div>
            ...
            return (
              <div key={item.id} className="grid grid-cols-12 items-center p-4 border-b border-[#F1F5F9] hover:bg-[#F8FAFC] transition-colors group">
                ...
                <div className="col-span-1 flex items-center justify-center pr-2">
                  {item.imageUrl ? (
                    <img src={item.imageUrl} alt={item.name} className="w-10 h-10 object-cover rounded-md border border-[#E2E8F0] shadow-sm" />
                  ) : (
                    <div className="w-10 h-10 bg-[#F1F5F9] rounded-md border border-[#E2E8F0] flex items-center justify-center text-[#CBD5E1]" title="Sin Imagen">
                      <ImageIcon size={18} />
                    </div>
                  )}
                </div>
                ...
                <div className="col-span-2 flex flex-col gap-1.5 items-start">
                  <span className="bg-[#F8FAFC] px-2 py-1 border border-[#E2E8F0] text-[11px] font-black text-[#1E293B] rounded-none truncate max-w-full" title={item.code}>
                    {item.code}
                  </span>
                  {item.barcode && (
                    <span className="flex items-center gap-1 text-[8px] font-black text-[#64748B] tracking-wider truncate w-full" title="Código de Escáner">
                      <Barcode size={10} className="text-[#94A3B8]" />
                      {item.barcode}
                    </span>
                  )}
                </div>
                ...
                <div className="col-span-3 pr-4 flex flex-col gap-1">
                  <p className="font-black text-xs uppercase text-[#1E293B] leading-tight line-clamp-2" title={item.name}>
                    {item.name}
                  </p>
                  <span className="self-start text-[8px] font-black text-white bg-[#1E293B] px-1.5 py-0.5 tracking-widest rounded-none mt-0.5">
                    {item.category}
                  </span>
                </div>
