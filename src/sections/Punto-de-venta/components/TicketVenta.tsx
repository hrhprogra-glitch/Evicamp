...
export const TicketVenta: React.FC<Props> = ({ 
  ...
  // NUEVO COMPONENTE: Input Inteligente para Kilos y Gramos
const InputPeso = ({ item, updateQuantity }: { item: CartItem, updateQuantity: (id: string, qty: number) => void }) => {
  const [val, setVal] = React.useState(item.cartQuantity.toFixed(3));
  
  React.useEffect(() => {
    if (parseFloat(val) !== item.cartQuantity) {
      setVal(item.cartQuantity.toFixed(3));
    }
  }, [item.cartQuantity]);
  ...
  const handleBlur = () => {
    ...
    if (!isNaN(num) && num > 0) {
       ...
       setVal(num.toFixed(3));
    } else {
       // Si el usuario borra todo o pone 0, regresa al peso anterior para evitar errores
       setVal(item.cartQuantity.toFixed(3));
    }
  };
  ...
  return (
    <input 
       ...
       className="w-full h-full text-center text-[12px] font-black text-[#3B82F6] outline-none bg-transparent cursor-text"
       title="Escribe el peso y presiona Enter"
    />
  );
};
  // NUEVO COMPONENTE: Input Inteligente para Precio (Consumo)
const InputPrecio = ({ item, updatePrice }: { item: CartItem, updatePrice: (id: string, price: number) => void }) => {
  ...
  const handleBlur = () => {
    ...
    if (!isNaN(num) && num >= 0) {
       ...
       setVal(num.toFixed(2));
    } else {
       setVal(item.price.toFixed(2));
    }
  };
  ...
  return (
    <div className="flex items-center gap-1 mt-0.5" title="Haz clic para editar el precio">
      ...
      <input 
         type="number" step="0.10" value={val} onChange={(e) => { setVal(e.target.value); const num = parseFloat(e.target.value); if (!isNaN(num) && num >= 0) updatePrice(item.id, num); }} onBlur={handleBlur} onKeyDown={(e) => e.key === 'Enter' && handleBlur()}
  ...
  return (
    <div className="w-[380px] flex flex-col bg-white border-2 border-[#1E293B] shadow-[8px_8px_0_0_#1E293B] shrink-0 relative overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-white p-4 flex flex-col relative border-b-2 border-[#1E293B]">
        <div className="grid grid-cols-12 gap-2 text-[9px] font-black text-[#64748B] uppercase tracking-widest border-b-2 border-[#E2E8F0] pb-2 mb-2">
          ...
          <div className="flex flex-col gap-2">
            {cart.map((item, index) => (
              <div key={index} className="grid grid-cols-12 gap-2 text-xs font-bold text-[#1E293B] border-b border-dashed border-[#CBD5E1] pb-2 items-center">
                ...
                <div className="col-span-3 flex items-center justify-between border-2 border-[#1E293B] bg-[#FFFFFF] h-8 overflow-hidden">
                  {item.unit === 'KG' ? (
                    ...
                    <div className="flex-1 flex items-center justify-center h-full bg-[#F8FAFC]">
                      <span className="text-[10px] font-black text-[#94A3B8] tracking-widest">---</span>
                    </div>
                  ) : (
                    <>
                      <button onClick={() => updateQuantity(item.id, item.cartQuantity - 1)} className="w-6 h-full flex items-center justify-center hover:bg-[#1E293B] hover:text-white transition-colors cursor-pointer border-r-2 border-[#1E293B]">
                        <Minus size={12} strokeWidth={4} />
                      </button>
                      <span className="flex-1 text-center text-[10px] font-black text-[#1E293B]">{item.cartQuantity}</span>
                      <button onClick={() => updateQuantity(item.id, item.cartQuantity + 1)} className="w-6 h-full flex items-center justify-center hover:bg-[#1E293B] hover:text-white transition-colors cursor-pointer border-l-2 border-[#1E293B]">
                ...
                <div className="col-span-5 flex flex-col min-w-0 pr-1 pl-1">
                  <span className="truncate uppercase font-black text-[#1E293B]" title={item.name}>{item.name}</span>
                  {item.unit === 'CONSUMO' ? (
                    <InputPrecio item={item} updatePrice={updatePrice} />
                  ) : (
                    <span className="text-[9px] text-[#64748B]">S/ {item.price.toFixed(2)} c/u</span>
                  )}
                </div>
                
                <div className="col-span-3 text-right font-black text-[#1E293B]">
      ...
      <div className="bg-[#F8FAFC] shrink-0 p-5 flex flex-col gap-4">

        <div className="flex justify-between items-end mt-1">
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#1E293B]">TOTAL A COBRAR</span>
        ...
        <button 
          ...
          disabled={cart.length === 0}
          className="w-full mt-2 bg-[#10B981] text-[#1E293B] py-4 border-2 border-[#1E293B] font-black text-sm uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:bg-[#1E293B] hover:text-[#10B981] disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer shadow-[4px_4px_0_0_#1E293B] hover:shadow-none hover:translate-x-[4px] hover:translate-y-[4px]"
        >
          <Banknote size={20} /> Proceder al Pago
        </button>
      </div>
    </div>
  );
